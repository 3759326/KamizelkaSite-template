			...
			foreach ($results as $result) {
				if ($result['image']) {
					$image = $this->model_tool_image->resize($result['image'], $this->config->get('config_image_product_width'), $this->config->get('config_image_product_height'));
                                        ...
                                        if (isset($settings['labelimage_productetc']) && $settings['labelimage_productetc']=='1'){
                                            if ($result['topleft_png'] || $result['topright_png'] || $result['bottomleft_png'] || $result['bottomright_png']){
                                                require_once DIR_APPLICATION . '/model/tool/labelimage.php';
                                                $label = new Labelimage();
                                                $image = $label->setLabelImage($image, $result['topleft_png'], $result['topright_png'], $result['bottomleft_png'], $result['bottomright_png'],$http,$result['image'],$result['product_id'], $this->config->get('config_image_product_width'), $this->config->get('config_image_product_height'));
                                            }
                                        }
                                        //end
