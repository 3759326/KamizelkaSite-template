	...
	public function confirm($order_id, $order_status_id, $comment = '', $notify = false) {
		...
		if ($order_info && !$order_info['order_status_id']) {
			...
			if ($this->config->get('config_alert_mail')) {
				...
				if( !empty($order_info['payment_lastname']) ) $text .= 'Покупатель: ' . $order_info['payment_lastname'] . ' '  .$order_info['payment_firstname'] . "\n";
				if( !empty($order_info['email']) )     $text .= 'E@mail: ' . $order_info['email'] . "\n";
				if( !empty($order_info['telephone']) ) $text .= 'Телефон: ' . $order_info['telephone'] . "\n";
				$text .='-----------------------------------'."\n";
				if( !empty($order_info['shipping_city']) ) $text .= 'Город: ' . $order_info['shipping_city'] . "\n";
				if( !empty($order_info['shipping_address_1']) ) $text .= 'Адрес: ' . $order_info['shipping_address_1'] . "\n";
				if( !empty($order_info['shipping_address_2']) ) $text .= 'Адрес: ' . $order_info['shipping_address_2'] . "\n";
				if( !empty($order_info['payment_method']) ) $text .= 'Оплата: ' . $order_info['payment_method'] . "\n";
				if ($order_info['comment']) {
					$text .= $language->get('text_new_comment') . "\n";
					$text .= $order_info['comment'] . "\n\n";
				}
				//$text .= 'IP: http://geoiptool.com/ru/?IP=' . $order_info['ip'] . "\n\n";
				$text .= $language->get('text_new_products') . "\n";
